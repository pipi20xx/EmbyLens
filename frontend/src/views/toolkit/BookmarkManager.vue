<template>
  <div class="bookmark-manager-full">
    <n-card :bordered="false" size="small" class="manager-card" content-style="padding: 0;">
      ...
      <div class="header-toolbar">
        ...
        <n-space :size="8">
          ...
          <n-button secondary size="small" @click="$refs.fileInput.click()" class="toolbar-btn">
    ...
    <!-- 弹窗 -->
    <n-modal v-model:show="showAddBookmarkModal" preset="card" :title="editingItem ? '编辑书签' : '添加书签'" style="width: 440px" class="rounded-2xl shadow-2xl">
      <n-form label-placement="top" size="small">
        ...
        <n-form-item label="图标 URL">
          <n-input v-model:value="form.icon" placeholder="自动填充或手动输入" />
        </n-form-item>
      </n-form>
      <template #footer>
        <n-space justify="end">
          <n-button @click="closeBookmarkModal">取消</n-button>
          <n-button type="primary" :disabled="!form.title" @click="saveBookmark" style="padding: 0 24px">保存</n-button>
        </n-space>
      </template>
    </n-modal>
    ...
    <n-modal v-model:show="showAddFolder" preset="card" title="新建文件夹" style="width: 360px" class="rounded-2xl">
      <n-form label-placement="top" size="small">
        <n-form-item label="文件夹名称">
          <n-input v-model:value="folderName" placeholder="例如：开发工具" @keyup.enter="saveFolder" />
        </n-form-item>
      </n-form>
      <template #footer>
        <n-space justify="end">
...
const {
  ...
  saveBookmark, saveFolder, autoFetchTitle, autoFetchIcon, handleImportHtml, onDragStart, onDragEnter, onDragEnd
} = useBookmarkManager()
</script>
...
.header-icon-box {
  ...
  background: rgba(var(--primary-color-rgb, 32, 128, 240), 0.1);
  border-radius: 8px;
...
.sidebar-tree {
  ...
  --n-node-color-active: rgba(var(--primary-color-rgb, 32, 128, 240), 0.1);
  --n-node-text-color: rgba(255, 255, 255, 0.6);
...
.action-btn.delete:hover { color: #f5222d; }
